...
pub mod slinear11 {
    pub fn from(val: f32, exp: i8) -> u16 {
        ...
        let lsb: f32;
        if exp.is_positive() {
            lsb = base.pow(exp as u32) as f32;
        } else {
            lsb = 1. / base.pow(exp.abs() as u32) as f32;
        }

        let mant: i32 = (val / lsb as f32) as i32;
        let mant = mant & 0x07FF;
        let exp: u32 = (exp & 0x1F) as u32;
        return (mant as u32 | (exp << 11)) as u16;
    }
    ...
    pub fn to(val: u16) -> f32 {
        ...
        if exp.is_positive() {
            ...
            mant as f32 / base.pow(exp.abs() as u32) as f32
        }
    }
}
